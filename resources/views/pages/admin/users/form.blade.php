...
<div class="relative w-full" x-data="{ isUploading: false, progress: 0 }" x-on:livewire-upload-start="isUploading = true; progress = 0"
    ...
    <form wire:submit.prevent="submit" x-data="imgPreview({{ $image ? "'$image'" : 'null' }})"
        ...
        <x-form-input2 id="usernameInp" wire:model.defer="user.username" value="{{ $user->username }}">Username
        </x-form-input2>
        <x-form-input2 id="mailInput" type="mail" wire:model.defer="user.email" value="{{ $user->email }}">Email
        </x-form-input2>
